...
fn load_custom_shortcuts() -> Vec<CustomShortcut> {
    match fs::read_to_string(CUSTOM_SHORTCUTS_PATH) {
        Ok(content) => {
            match serde_json::from_str::<CustomShortcutsConfig>(&content) {
...
pub fn execute(name: &str, args: &Value, session_executor: fn(&str, &Value) -> Value) -> Value {
    match name {
        "shortcut_list" => {
            // Built-in shortcuts
            let mut shortcuts: Vec<Value> = SHORTCUTS.iter().map(|(name, cmds, desc)| {
                json!({
                    "name": name,
                    "commands": cmds,
                    "description": desc,
        ...
        "shortcut_run" => {
            ...
            if let Some((_, commands, desc)) = builtin {
                return run_shortcut_commands(shortcut_name, commands.to_vec(), desc, "builtin", session, session_executor);
            }
            ...
            if let Some(cs) = custom.iter().find(|s| s.name == shortcut_name) {
                ...
                return run_shortcut_commands(shortcut_name, cmds, &cs.description, "custom", session, session_executor);
            }
        ...
        "shortcut_chain" => {
            ...
            for (i, cmd) in commands.iter().enumerate() {
                ...
                if checkpoint {
                    ...
                    let _ = std::fs::write(&checkpoint_file, serde_json::to_string_pretty(&checkpoint_data).unwrap());
                }
